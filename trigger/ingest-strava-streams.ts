...
type IngestStreamsPayload = {
  userId: string
  workoutId: string
  activityId: number
}

export async function runIngestStravaStreams(payload: IngestStreamsPayload) {
...
registerTaskHandler('ingest-strava-streams', runIngestStravaStreams)

export const ingestStravaStreams = task({
  id: 'ingest-strava-streams',
  queue: userIngestionQueue,
  maxDuration: 900, // 15 minutes
  run: runIngestStravaStreams
})
