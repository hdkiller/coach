...
export const adaptTrainingPlanTask = task({
  ...
  run: async (payload: { planId: string; userId: string; adaptationType: string; context?: string }) => {
    ...
    const now = new Date();
    const currentBlock = plan.blocks.find(b => new Date(b.startDate) <= now && new Date(b.startDate).getTime() + (b.durationWeeks * 7 * 24 * 3600 * 1000) > now.getTime());
    ...
    if (adaptationType === "RECALCULATE_WEEK") {
      ...
      const currentWeek = currentBlock.weeks.find(w => new Date(w.startDate) <= now && new Date(w.endDate) >= now);
