---
title: Athletes
description: Guides and resources for athletes using Coach Watts to optimize their training.
---

Welcome to the Coach Watts athlete guides. This section is designed to help you get the most out of our platform.

## Start here

- **[Athlete User Guide](/documentation/athletes/user-guide)** — tour of Dashboard, Chat, Activities, Plan, Library, and settings
- **[Getting Started](/documentation/athletes/getting-started)** — account setup, consent, and first connection
- **[Dashboard & Daily Flow](/documentation/athletes/dashboard-daily-flow)** — your home screen and morning routine
- **[AI Chat & Supported Tools](/documentation/athletes/chat-interaction)** — data-connected actions, analysis, memory, media, voice, and conversation controls
- **[Account & Profile Setup](/documentation/athletes/account-profile)** — FTP, zones, availability, nutrition settings

## Using Coach Watts

- **[Features Overview](/documentation/athletes/features)** — map of all platform capabilities
- **[Training Plans](/documentation/athletes/training-plans)** — goal-driven periodized plans
- **[Daily Recommendations](/documentation/athletes/recommendations)** — AI guidance for today
- **[Activities & Workout Analysis](/documentation/athletes/activities-workouts)** — calendar, scores, sharing
- **[Activity Calendar & Workout Management](/documentation/athletes/activity-management)** — moving, matching, merging, scheduling, and cleanup
- **[Duplicate Workouts & Versions](/documentation/athletes/duplicate-workouts)** — automatic and manual deduplication, primary selection, and corrections
- **[Workout Detail Guide](/documentation/athletes/workout-details)** — analysis, tags, sections, adherence, sharing, comparison, and exports
- **[Nutrition](/documentation/athletes/nutrition)** — daily fueling and meal tracking
- **[Performance & Trends](/documentation/athletes/performance)** — CTL/ATL, bests, analytics
- **[Goals & Events](/documentation/athletes/goals-events)** — races and fitness targets
- **[Reports](/documentation/athletes/reports)** — weekly and monthly AI analysis
- **[Recovery & Wellness](/documentation/athletes/recovery-wellness)** — HRV, sleep, readiness
- **[Connect AI Assistants (MCP)](/documentation/athletes/mcp-server)** — Cursor and other AI tools
- **[Workout, Exercise & Plan Libraries](/documentation/athletes/library)** — reusable training content and folders
- **[Strength Training](/documentation/athletes/strength-training)** — exercises, Gym templates, schedule, and logging
- **[Structured Workouts](/documentation/athletes/structured-workouts)** — create, schedule, export, complete, and compare
- **[Working with a Coach](/documentation/athletes/working-with-a-coach)** — connect, permissions, and revoke access
- **[Analytics Explorer & Workout Comparison](/documentation/athletes/analytics-tools)** — custom charts and session comparison

## Settings & data

- **[Settings](/documentation/athletes/settings)** — complete settings index
- **[Data Management](/documentation/athletes/data-management)** — inspect and remove workout, wellness, event, and nutrition records
- **[Connected Apps & Sync Settings](/documentation/athletes/settings/apps-sync)** — provider access, ingestion, and source conflicts
- **[Data, Privacy & Danger Zone](/documentation/athletes/settings/data-privacy)** — exports, category wipes, public presence, and account deletion

## Data & metrics

- **[Integrations](/documentation/athletes/integrations)** — Strava, Garmin, Oura, and more
- **[Nutrition & Fueling Logic](/documentation/athletes/fueling-logic)** — how carb targets are calculated
- **[Metrics & Scoring](/documentation/athletes/metrics-scoring)** — TSS, CTL, AI scores

## Help

- **[FAQ](/documentation/help/faq)** — troubleshooting
- **[Glossary](/documentation/help/glossary)** — training terminology
- **[Account & Billing](/documentation/help/account-billing)** — plans, portal, cancel, quotas

Select a topic from the sidebar to begin.
