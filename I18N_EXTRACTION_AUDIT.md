...
- Completed full extraction pass for:
  - `app/pages/profile/settings.vue` (availability card header)
  - `app/components/profile/NutritionSettings.vue`
  - `app/components/profile/MeasurementsSettings.vue`
  - `app/components/profile/SportSettings.vue`
  ...
  - dashboard navbar controls: `TriggerMonitorButton`, `NotificationDropdown`, `ReleaseNotification`
  - `app/components/dashboard/ShareCoachWattsModal.vue` (Help Us Grow share flow modal copy/toasts/labels)
  - `app/pages/settings/billing.vue` full pass (trust signals, comparison table rows, FAQ entries, plan label text)
