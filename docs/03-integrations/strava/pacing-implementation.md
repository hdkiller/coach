...
export function calculatePaceVariability(velocityData: number[]) {
  ...
  const variance = velocityData.reduce((sum, v) => sum + Math.pow(v - mean, 2), 0) / velocityData.length
...
export const ingestStravaStreams = task({
  ...
  run: async (payload: { userId: string; workoutId: string; activityId: number }) => {
    ...
    const streams = await fetchStravaActivityStreams(
      integration,
      payload.activityId,
      ['time', 'distance', 'velocity_smooth', 'heartrate', 'cadence', 'watts', 'altitude']
    )
...
const { data: streams, pending: loading, error } = await useFetch(
