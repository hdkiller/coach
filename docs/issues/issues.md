...
| -------- | ----- | --------------------------------------------------- |
| Critical | 14    | Incl. Garmin 312–313; structure-gen criticals fixed |
| High     | 99    | Incl. open **012** (AI-in-triggers), CI/CD **370**  |
| Medium   | 215   | Largest bucket — email, push, nutrition, CI/CD      |
| Low      | 49    | i18n / polish / housekeeping                        |
...
## Issues 001–038 (structure generation)
...
### Structure generation — still open

- **[012](./012-ai-in-triggers-architecture-rethink.md)** — Durable `PlannedWorkout` generation status; broader AI-in-triggers architecture (shared AI timeouts shipped in PR #222).

### Structure generation — clusters (historical)

Speed/prompt (033–038), trigger monitor/tags (002, 027–032), chat+structure duplicates/failures (008, 013–016, 023–024): see individual issue files. Merged in PRs [#214](https://github.com/hdkiller/coach/pull/214)–[#222](https://github.com/hdkiller/coach/pull/222) (2026-07-08).

### Structure generation — architecture

Planned Workout Details / Plan Dashboard / Chat / `generate-training-block` / `generate-ad-hoc-workout` → Trigger.dev `generate-structured-workout` / `adjust-structured-workout` (`maxDuration` 180s) → Gemini (`45s` timeout per attempt). Chat turns use a separate 60s execution timeout; heavy structure work is offloaded to Trigger.dev.

Production defect family: zero-step structured workouts (`step_count = 0`). Support cluster 3 in [support-ticket-task-list-2026-06-16.md](../06-plans/support-ticket-task-list-2026-06-16.md).

---

## Issues 039–061 (dashboard, billing, auth foundations)
...
- [SENTRY-ISSUES.md](../../SENTRY-ISSUES.md) — Sentry living tracker
