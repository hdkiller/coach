<script setup lang="ts">
  ...
  const modelsByFamily = computed(() => {
    const map: Record<string, typeof MODEL_LIST> = {
      flash: MODEL_LIST.filter((m) => m.family === 'flash'),
      pro: MODEL_LIST.filter((m) => m.family === 'pro')
    }
    return map
  })

  function getModelsForFamily(family: string) {
    return modelsByFamily.value[family] || []
  }
  ...
  function onOverrideFamilyChange() {
...
<template>
  <UDashboardPanel>
    <template #header>
      <UDashboardNavbar title="LLM Tuning">
        <template #leading>
    ...
    <template #body>
      <div class="p-4 sm:p-6 flex-1 overflow-y-auto max-w-7xl mx-auto w-full">
        ...
        <div v-else class="space-y-12 pb-24">
          <div v-for="level in sortedSettings" :key="level.id" class="space-y-4">
            <!-- Level Header -->
                ...
                <div class="space-y-5">
                  ...
                  <UFormField label="Exact Model">
                    <USelect
                      ...
                      :items="modelsByFamily[level.model]"
                      value-key="value"
                      class="w-full"
              ...
              <div class="space-y-4">
                ...
                >
                  <USelectMenu
                    ...
                    :items="modelsByFamily[overrideState.model]"
                    value-key="value"
                    class="w-full"
                  />
                </UFormField>
