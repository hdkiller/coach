    ...
    <div v-else-if="error" class="text-center py-12">
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-8 max-w-md mx-auto">
        ...
        <p class="text-gray-600 dark:text-gray-400 mb-6">
          {{ error.message || 'This shared plan link is invalid or expired.' }}
        </p>
          ...
          >
            ...
            <div class="p-4 flex-1 space-y-3">
              ...
              <NuxtLink
                ...
                :to="
                  workout.shareToken?.token
                    ? `/share/planned-workout/${workout.shareToken.token}`
                    : '#'
                "
                :class="[
              ...
              >
                ...
                <UIcon
                  v-if="workout.shareToken?.token"
                  name="i-heroicons-arrow-top-right-on-square"
                  class="w-4 h-4 text-gray-400 opacity-0 group-hover:opacity-100 transition-opacity absolute top-3 right-3"
                />
              </NuxtLink>
            </div>
          </div>
...
<script setup lang="ts">
  ...
  const pageDescription = computed(() => {
    if (plan.value) {
      return `Check out this training plan: ${plan.value.name}. ${plan.value.description || ''}`.substring(
        0,
        160
      )
    }
