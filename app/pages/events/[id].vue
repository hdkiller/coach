        ...
        <div v-else-if="event" class="space-y-6">
          ...
          <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            ...
            <div class="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div v-if="event.distance" class="rounded-lg p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800">
                <div class="text-xs text-blue-600 dark:text-blue-400 mb-1">Distance</div>
                <div class="text-xl font-bold text-blue-900 dark:text-blue-100">{{ event.distance }} <span class="text-sm font-normal">km</span></div>
              </div>
              <div v-if="event.elevation" class="rounded-lg p-3 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800">
                <div class="text-xs text-green-600 dark:text-green-400 mb-1">Elevation</div>
                <div class="text-xl font-bold text-green-900 dark:text-green-100">{{ event.elevation }} <span class="text-sm font-normal">m</span></div>
              </div>
              <div v-if="event.expectedDuration" class="rounded-lg p-3 bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800">
                <div class="text-xs text-purple-600 dark:text-purple-400 mb-1">Est. Duration</div>
                <div class="text-xl font-bold text-purple-900 dark:text-purple-100">{{ event.expectedDuration }} <span class="text-sm font-normal">h</span></div>
              </div>
              <div class="rounded-lg p-3 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
                <div class="text-xs text-gray-600 dark:text-gray-400 mb-1">Location</div>
                <div class="text-sm font-bold text-gray-900 dark:text-white truncate" :title="formatLocation(event)">
            ...
            <div v-if="event.description" class="mt-6">
              ...
              <p class="text-sm text-gray-600 dark:text-gray-400 whitespace-pre-wrap">{{ event.description }}</p>
            </div>
          </div>

          <!-- Goals Section -->
          <div v-if="event.goals && event.goals.length > 0" class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
...
useHead(() => {
  ...
  return {
    ...
    meta: [
      { name: 'description', content: `Details for ${event.value.title}` }
    ]
  }
})
