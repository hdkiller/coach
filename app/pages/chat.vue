  ...
  <UDashboardPanel id="chat" :ui="{ body: 'p-0' }">
    ...
    <template #header>
      <UDashboardNavbar :title="currentRoomName">
        <template #leading>
          ...
          <UButton color="neutral" variant="ghost" icon="i-heroicons-clock" @click="isRoomListOpen = true" />
        </template>
        <template #right>
          <ClientOnly>
            <DashboardTriggerMonitorButton />
          </ClientOnly>
          <UButton to="/settings/ai" icon="i-heroicons-cog-6-tooth" color="neutral" variant="outline" size="sm"
            class="font-bold" aria-label="AI Settings">
            <span class="hidden sm:inline">Settings</span>
          </UButton>
          <UButton color="primary" variant="solid" icon="i-heroicons-chat-bubble-left-right" aria-label="New Chat"
            size="sm" class="font-bold" @click="createNewChat">
    ...
    <template #body>
      <div class="flex h-full">
        ...
        <ChatSidebar v-model:is-open="isRoomListOpen" :rooms="rooms" :current-room-id="currentRoomId"
          :loading="loadingRooms" @select="selectRoom" @delete="deleteRoom" @rename="renameRoom" />

        <!-- Chat Area -->
        <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
          <!-- Read-only Banner -->
          <div v-if="isCurrentRoomReadOnly"
            class="p-2 sm:p-4 border-b border-warning-200 bg-warning-50 dark:border-warning-900/50 dark:bg-warning-950/20">
            <UAlert color="warning" variant="subtle" icon="i-heroicons-information-circle" title="Legacy Chat"
              ...
              <template #actions>
                <UButton color="warning" variant="outline" size="xs" label="New Chat" @click="createNewChat" />
              </template>
            </UAlert>
          </div>

          <!-- Messages -->
          <ChatMessageList :messages="chatMessages as any" :status="chatStatus" :loading="loadingMessages"
            @tool-approval="onToolApproval" />

          <!-- Input -->
          <ChatInput ref="chatInputRef" v-model="input" :status="chatStatus" :error="chat.error"
            :disabled="isCurrentRoomReadOnly" @submit="onSubmit" />
        </div>
      </div>
    </template>
  </UDashboardPanel>
