<template>
  <UDashboardPanel id="ai-log-detail">
    <template #header>
      <UDashboardNavbar title="AI Usage Detail">
        <template #leading>
          <UButton icon="i-heroicons-arrow-left" color="neutral" variant="ghost" to="/ai/logs">
            Back to Logs
          </UButton>
        </template>
      </UDashboardNavbar>
        ...
        <template v-else-if="data">
          ...
          <UCard v-if="data.responseFull || data.responsePreview">
            ...
            <div v-if="!data.responseFull && data.responsePreview" class="mt-2 text-xs text-muted">
              Note: Only preview available (first 500 characters). Full responses are stored for
              newer records.
            </div>
          </UCard>
        </template>
