...
<script setup lang="ts">
  ...
  async function confirmClearAll() {
    clearing.value = true
    try {
      const res: any = await $fetch<any, string & {}>('/api/recommendations/clear', {
        method: 'DELETE'
      })
  ...
  async function refreshAdvice() {
    ...
    try {
      const res: any = await $fetch<any, string & {}>('/api/recommendations/generate', {
        method: 'POST'
      })
  ...
  async function togglePin(rec: any) {
    ...
    try {
      await $fetch<any, string & {}>(`/api/recommendations/${rec.id}`, {
        method: 'PATCH',
