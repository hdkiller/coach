<template>
  <div class="space-y-4">
    <div class="flex items-center justify-between mb-2">
      ...
      <span class="text-[10px] font-bold text-gray-400 uppercase tracking-tighter">{{ windows.length }} Active
        Windows</span>
    </div>
    ...
    <div class="relative py-4">
      <template v-for="(window, index) in windows" :key="index">
        ...
        <NutritionWorkoutEventCard v-if="window.type === 'WORKOUT_EVENT'" :workout="window.workout"
          :start-time="window.startTime" :end-time="window.endTime" :fuel-state="getWorkoutFuelState(window.workout)" />

        <!-- Fueling Window -->
        <NutritionWindowBlock v-else :type="window.type" :title="formatTitle(window)" :start-time="window.startTime"
          :end-time="window.endTime" :target-carbs="window.targetCarbs" :target-protein="window.targetProtein"
          :target-fat="window.targetFat" :target-fluid="window.targetFluid" :target-sodium="window.targetSodium"
          :items="window.items" :supplements="window.supplements" :is-locked="isLocked"
