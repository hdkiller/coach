<template>
  <div ref="dayCell" class="min-h-[120px] p-2 bg-white dark:bg-gray-900 transition-colors flex flex-col relative"
    :class="{
    ...
    }" @dragover.prevent="onDayDragOver" @dragenter.prevent="onDayDragEnter" @dragleave="onDayDragLeave"
    @drop="onDayDrop">
    <!-- Date Number & Wellness Metrics -->
    <div class="flex items-center justify-between mb-2">
      <div class="flex items-center gap-2">
        <div class="relative">
          <span class="text-xs font-semibold flex items-center justify-center w-6 h-6" :class="{
          ...
          <!-- Fuel State Dot -->
          <div v-if="fuelState"
            class="absolute -top-1 -right-1 w-2.5 h-2.5 rounded-full border-2 border-white dark:border-gray-900" :class="{
        ...
        <button v-if="dayWellness"
          ...
          :title="'View wellness details'" @click="$emit('wellness-click', date)">
    ...
    <div class="space-y-1 flex-1">
      <button v-for="activity in displayActivities" :key="activity.id"
        ...
        }" @click="$emit('activity-click', activity)" @dragover.prevent="onDragOver" @dragleave="onDragLeave"
        ...
        " class="absolute top-0 right-0 p-1 opacity-0 group-hover:opacity-100 cursor-grab active:cursor-grabbing z-10 hover:bg-black/5 rounded-bl"
          :draggable="true" @dragstart.stop="(e) => onDragStart(e, activity)" @click.stop>
          <UIcon name="i-heroicons-bars-2" class="w-3 h-3 text-gray-400" />
        </div>

        <!-- Status Dot -->
        <div class="flex items-start gap-1.5">
          <div class="w-2 h-2 rounded-full mt-0.5 flex-shrink-0" :class="{
          ...
          <div class="flex-1 min-w-0">
            ...
            <div class="font-medium truncate flex items-center gap-1" :title="activity.title">
              ...
              <UIcon v-if="activity.isWeeklyNote" name="i-heroicons-calendar-days" class="w-3 h-3 text-primary-500"
                title="Weekly Note" />
            </div>

            <!-- Note Category -->
            <div v-if="activity.source === 'note' && activity.category"
            ...
            " class="flex items-center gap-1.5 text-[10px] text-gray-500 dark:text-gray-400 mt-0.5">
              ...
              <span class="inline-flex items-center gap-0.5">
                <template v-if="activity.tss || activity.trimp || activity.plannedTss">
                  <span class="w-3 h-0.5 rounded-full flex-shrink-0" :class="{
            ...
            <div v-if="activity.source === 'completed' && (activity.ctl || activity.atl)"
              ...
              <UTooltip v-if="activity.ctl && activity.atl"
            ...
            <div v-if="activity.linkedPlannedWorkout"
            ...
            <div v-if="activity.source === 'planned' && activity.structuredWorkout" class="mt-1.5">
              <MiniWorkoutChart :workout="activity.structuredWorkout" :preference="getPreferredMetric(getActivityZones(activity), {
            ...
            <div v-if="activity.source === 'completed' && activity.hasStreams" class="mt-1.5">
              <MiniZoneChart :workout-id="activity.id" :auto-load="false" :stream-data="streams?.[activity.id]"
