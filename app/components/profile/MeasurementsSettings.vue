...
<script setup lang="ts">
  ...
  async function saveMeasurement() {
    ...
    try {
      ...
      await $fetch<unknown, string & {}>('/api/body-measurements', {
        method: 'POST',
        body: {
  ...
  async function deleteMeasurement(entry: any) {
    ...
    try {
      await $fetch<unknown, string & {}>(`/api/body-measurements/${entry.id}`, {
        method: 'PATCH',
        body: {
          isDeleted: true
        }
  ...
  async function saveEdit() {
    ...
    try {
      ...
      await $fetch<unknown, string & {}>(`/api/body-measurements/${editingEntry.value.id}`, {
        method: 'PATCH',
        body: {
          value: nextValue,
