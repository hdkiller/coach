  ...
  <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
    ...
    <div v-if="workout.structuredWorkout?.exercises?.length" class="space-y-6">
       <div v-for="(group, gIndex) in groupedExercises" :key="gIndex">
           <h4 v-if="group.name" class="font-semibold text-gray-900 dark:text-white mb-2 uppercase text-xs tracking-wider">{{ group.name }}</h4>
           <div class="space-y-3">
               <div v-for="(exercise, eIndex) in group.exercises" :key="eIndex" class="flex items-center p-3 bg-gray-50 dark:bg-gray-900 rounded-lg">
                   <div class="w-8 h-8 rounded-full bg-purple-100 dark:bg-purple-900 text-purple-600 dark:text-purple-300 flex items-center justify-center font-bold text-sm mr-4 flex-shrink-0">
                       {{ Number(eIndex) + 1 }}
                   </div>
                   <div class="flex-1">
                       ...
                       <div v-if="exercise.notes" class="text-xs text-gray-500 italic mt-1">{{ exercise.notes }}</div>
                   </div>
               </div>
           </div>
       </div>
