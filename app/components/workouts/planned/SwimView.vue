    ...
    <div class="flex justify-between items-center mb-4">
     ...
     <div v-if="workout.structuredWorkout?.steps" class="space-y-4">
        <div v-for="(step, index) in workout.structuredWorkout.steps" :key="index" class="flex items-center p-3 bg-gray-50 dark:bg-gray-900 rounded-lg">
             <div class="w-8 h-8 rounded-full bg-cyan-100 dark:bg-cyan-900 text-cyan-600 dark:text-cyan-300 flex items-center justify-center font-bold text-sm mr-4">
                 {{ Number(index) + 1 }}
             </div>
             <div class="flex-1">
      ...
      <div v-else class="text-center py-8 text-muted">
        No structured swim steps available.
    </div>
  </div>
</template>
