<template>
  <div class="efficiency-trend-chart">
    ...
    <div v-else-if="!trendData || !trendData.trends || trendData.trends.length === 0" class="text-center py-12">
    ...
    <div v-else class="space-y-6">
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
        <div class="flex items-center justify-between mb-4">
          <h3 class="text-sm font-semibold text-gray-700 dark:text-gray-300">Aerobic Efficiency Factor (EF)</h3>
          <div class="flex items-center gap-2">
            <span class="text-xs text-gray-500">Trend:</span>
            <UBadge :color="trendDirection === 'up' ? 'primary' : trendDirection === 'down' ? 'error' : 'neutral'" size="xs" variant="subtle">
...
const trendDirection = computed(() => {
  ...
  const startAvg = trends.slice(0, window).reduce((sum: number, t: any) => sum + t.efficiencyFactor, 0) / window
  const endAvg = trends.slice(-window).reduce((sum: number, t: any) => sum + t.efficiencyFactor, 0) / window
