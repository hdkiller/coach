    ...
    <template #header>
      <div class="flex items-center justify-between w-full">
        ...
        <div class="flex items-center gap-4">
          ...
          <div
            v-if="latestStats"
            class="hidden md:flex items-center gap-4 border-r border-gray-200 dark:border-gray-800 pr-4 mr-2"
          >
            ...
            <div class="text-center">
              <div class="text-[10px] font-black text-gray-400 uppercase">
                {{ settings.baselineDays }}d Base
              </div>
              <div class="text-sm font-black text-gray-600 dark:text-gray-300">
                {{ latestStats.baseline }}ms
              </div>
            </div>
    ...
    const datasets: any[] = [
      ...
      },
      {
        ...
        metadata: data.map((w, i) => {
          const events = []
          if (
            settings.value.showAlcohol &&
            (w.tags?.toLowerCase().includes('alcohol') || w.rawJson?.alcohol > 0)
          )
            events.push('🍺 Alcohol')
          if (
            settings.value.showIllness &&
            (w.tags?.toLowerCase().includes('sick') ||
              w.tags?.toLowerCase().includes('illness') ||
              w.injury?.toLowerCase().includes('sick'))
          )
            events.push('🤒 Sickness')
          if (settings.value.showSleep && w.sleepHours > 0 && w.sleepHours < 6)
            events.push('😴 Poor Sleep')
