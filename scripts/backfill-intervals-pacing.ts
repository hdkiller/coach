...
async function backfillIntervalsPacing(options: BackfillOptions = {}) {
  ...
  if (dryRun) {
    ...
    for (const workout of workouts) {
      console.log(`   - ${workout.date.toISOString().split('T')[0]} | ${workout.type} | ${workout.title} (${workout.id})`)
    }
    return
  }
  ...
  for (const workout of workouts) {
    ...
    if (!integration) {
      console.log(`⚠️  [${processed}/${workouts.length}] No Intervals.icu integration for user ${workout.userId}`)
      skipped++
      continue
    }
    
    try {
      console.log(`🔄 [${processed}/${workouts.length}] Processing: ${workout.date.toISOString().split('T')[0]} | ${workout.type} | ${workout.title}`)
