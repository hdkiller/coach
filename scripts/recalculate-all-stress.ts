...
async function main() {
  ...
  const { calculateCTL, calculateATL, getStressScore } =
  ...
  try {
    ...
    for (const user of users) {
      ...
      console.log(
        `   📅 Found ${workouts.length} workouts. Starting from ${workouts[0].date.toISOString().split('T')[0]}`
      )
      ...
      for (let i = 0; i < workouts.length; i++) {
        ...
        if (
          ...
          atl = workout.atl
          console.log(
            `      🎯 Using Intervals.icu starting point: CTL=${ctl.toFixed(1)}, ATL=${atl.toFixed(1)}`
          )
          updatedCount++
        ...
        if (forceTss && workout.source !== 'intervals') {
          ...
          if (result.tss !== null) {
            ...
            if (workout.tss !== tss) {
              changedCount++
              console.log(
                `      🔄 Updated TSS for "${workout.source}" workout: ${workout.tss} -> ${tss}`
              )
            }
          }
        }
