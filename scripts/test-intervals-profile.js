...
async function testAthleteProfile() {
  ...
  if (athlete) {
    ...
    console.log(`  Age: ${athlete.icu_date_of_birth ? calculateAge(athlete.icu_date_of_birth) : 'N/A'} years`)
    console.log(`  Resting HR: ${athlete.icu_resting_hr || athlete.resting_hr || 'N/A'} bpm`)
    console.log(`  Location: ${[athlete.city, athlete.state, athlete.country].filter(Boolean).join(', ') || 'N/A'}`)
    ...
    if (athlete.icu_type_settings && athlete.icu_type_settings.length > 0) {
      athlete.icu_type_settings.forEach(typeSetting => {
        ...
        if (typeSetting.threshold_pace) console.log(`      Threshold Pace: ${typeSetting.threshold_pace} ${typeSetting.pace_units || ''}`)
      })
    } else {
  ...
  if (wellnessData.length > 0) {
    ...
    if (recentHrvValues.length > 0) {
      const avgHrv = (recentHrvValues.reduce((a, b) => a + b, 0) / recentHrvValues.length).toFixed(1)
  ...
  if (athlete) {
    ...
    if (athlete.icu_type_settings && athlete.icu_type_settings.length > 0) {
      ...
      const cyclingSettings = athlete.icu_type_settings.find(s =>
    ...
    const profile = {
      ...
      avgRecentHRV: wellnessData.length > 0 ?
        parseFloat(calculateAverage(wellnessData.map(d => d.hrv).filter(v => v != null))) : null,
