...
async function debugMultipleLogs(prisma: PrismaClient, options: any) {
  ...
  console.log(
    chalk.gray(
      `Fetching last ${limit} webhooks${options.status ? ` with status ${options.status}` : ''}...`
    )
  )
...
=== Webhook Log Details ===`)
  ...
  console.log(
    `Status:      ${log.status === 'PROCESSED' ? chalk.green(log.status) : log.status === 'FAILED' ? chalk.red(log.status) : chalk.yellow(log.status)}`
  )
  console.log(`Created At:  ${chalk.white(log.createdAt.toISOString())}`)
  console.log(
    `Processed At: ${log.processedAt ? chalk.white(log.processedAt.toISOString()) : chalk.gray('N/A')}`
  )
  if (log.error) {
...
=== Payload Analysis ===`)
  ...
  if (log.provider === 'intervals') {
    ...
    console.log(
      chalk.yellow(`Analysis logic for provider '${log.provider}' is not implemented yet.`)
    )
  }
}
...
--- Event #${index + 1} (${event.type}) ---`)
      ...
      case 'ACTIVITY_DELETED': {
        ...
        if (deletedActivityId) {
          ...
          if (exists) {
            ...
            console.log(
              chalk.green(`✅ Activity ${deletedActivityId} not found in DB (correctly deleted).`)
            )
          }
        }
        startDate = new Date(log.createdAt)
    ...
    if (startDate) {
      ...
      if (log.processedAt) {
        console.log(
          chalk.gray(
            `Checking for data updates after processedAt (${log.processedAt.toISOString()})...`
          )
        )
...
Updates since processing:`)
        console.log(
          `Workouts:        ${workoutsUpdated > 0 ? chalk.green(workoutsUpdated) : chalk.gray(0)}`
        )
        console.log(
          `Wellness:        ${wellnessUpdated > 0 ? chalk.green(wellnessUpdated) : chalk.gray(0)}`
        )
        console.log(
          `PlannedWorkouts: ${plansUpdated > 0 ? chalk.green(plansUpdated) : chalk.gray(0)}`
        )
        console.log(
          `Events:          ${eventsUpdated > 0 ? chalk.green(eventsUpdated) : chalk.gray(0)}`
        )
      } else {
